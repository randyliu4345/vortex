    ...
    always_ff @(posedge clk) begin
        ...
        if (dcr_wr_valid) begin
            case(dcr_wr_addr)
                ...
                // Shared memory size
                `VX_DCR_BASE_LMEM_SIZE:     smem_size <= dcr_wr_data;
                default: ; // ignore
            endcase
        end
    end
    ...
    always_ff @(posedge clk) begin
        if (reset) begin
            ...
            if (all_cta_sent) begin
                ...
                if (!kmu_bus_if.req_valid) begin
                    // Prepare and send one CTA block
                    kmu_bus_if.req_data.num_warps    <= total_warps;
                    kmu_bus_if.req_data.start_pc     <= kmu_data.pc;
                    kmu_bus_if.req_data.param        <= kmu_data.param;
                    kmu_bus_if.req_data.cta_x        <= counter_x;
                    kmu_bus_if.req_data.cta_y        <= counter_y;
                    kmu_bus_if.req_data.cta_z        <= counter_z;
                    kmu_bus_if.req_data.cta_id       <= counter_id;
                    kmu_bus_if.req_data.remain_mask  <= remain_mask;
                    kmu_bus_if.req_valid             <= 1;
                end
