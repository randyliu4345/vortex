...
- *L2mesh* - (simx only) enable L2 mesh hop latency between cores and L2 banks (`-DL2_MESH_ENABLE`).
...
### L2 mesh latency and core affinity (simx)

SimX can model a 2-D mesh over L2 banks: Manhattan distance between the requesting core (L2 input port) and the address-selected bank adds `L2_MESH_HOP_DELAY` cycles on each request and matching response.

Configure and run the motivating regression:

```bash
# from build/ after configure:
CONFIGS="-DL2_ENABLE -DL2_MESH_ENABLE -DNUM_CORES=4 -DSOCKET_SIZE=1 -DL2_NUM_BANKS=4" ../configure ...
make -C tests/regression/mesh_l2_affinity
./ci/blackbox.sh --driver=simx --app=mesh_l2_affinity --cores=4 --l2cache --l2mesh
```

The test compares **home** affinity (partition *i* on core *i*), **remote** pinning `(i+2)%4`, and **ANY**. Partition data is **bank-confined** (one L2 bank per partition): 16 floats per 256-byte granule so bank-select bits stay fixed. Layout is generated by `scripts/gen_mesh_l2_data.py` into `mesh_l2_data.h` (`python3 scripts/gen_mesh_l2_data.py --analyze` for expected hop mixes).
