...
Per-socket private L2 + socket mesh hops (`--l2socket`, usually with `--l2mesh`); features use socket-homed addresses (`gnn_layout.h`, `L2_SOCKET_REGION_LOG2=15`):

```bash
VORTEX_MESH_STATS=1 CONFIGS="-DL2_ENABLE -DL2_SOCKET_PRIVATE -DL2_MESH_ENABLE -DNUM_CORES=4" \
  ./ci/blackbox.sh --driver=simx --app=gnn_spmm --cores=4 --l2cache --l2socket --l2mesh
```

Shared L2 bank mesh only (`--l2mesh` without `--l2socket`):
